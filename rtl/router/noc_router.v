...
module noc_router #(
    ...
    genvar g;
    generate
        for (g = 0; g < 5; g = g + 1) begin : unpack
    ...
    function [2:0] route_of;
        ...
        begin
            if (dx < 0)      route_of = W;
            else if (dx > 0) route_of = E;
            else if (dy < 0) route_of = N;
            else if (dy > 0) route_of = S;
            else             route_of = L;
        end
    endfunction
    ...
    function signed [DX_W-1:0] next_dx;
        input signed [DX_W-1:0] dx;
        begin
            if (dx < 0)      next_dx = dx + 1'b1;
            else if (dx > 0) next_dx = dx - 1'b1;
            else             next_dx = dx;
        end
    endfunction
    ...
    function [2:0] cand_input;
        ...
        begin
            case (outp)
                N: case (slot) 2'd0: cand_input=S; 2'd1: cand_input=E; 2'd2: cand_input=W; default: cand_input=L; endcase
                S: case (slot) 2'd0: cand_input=N; 2'd1: cand_input=E; 2'd2: cand_input=W; default: cand_input=L; endcase
                E: case (slot) 2'd0: cand_input=N; 2'd1: cand_input=S; 2'd2: cand_input=W; default: cand_input=L; endcase
                W: case (slot) 2'd0: cand_input=N; 2'd1: cand_input=S; 2'd2: cand_input=E; default: cand_input=L; endcase
                default: case (slot) 2'd0: cand_input=N; 2'd1: cand_input=S; 2'd2: cand_input=E; default: cand_input=W; endcase
            endcase
        end
    endfunction
    ...
    always @(*) begin
        for (i = 0; i < 5; i = i + 1) begin
            ...
            end else if (in_valid[i] && (p_in_type[i] == HEAD || p_in_type[i] == HEADTAIL)) begin
                want_valid[i] = 1'b1;
        ...
        for (j = 0; j < 5; j = j + 1) begin
            ...
            for (s = 0; s < 4; s = s + 1) begin
                ...
                if (!found && want_valid[cand_input(j[2:0], idx2)] &&
                    want_out[cand_input(j[2:0], idx2)] == j[2:0]) begin
                    found       = 1'b1;
                    chosen_slot = idx2;
                end
            end
    ...
    always @(*) begin
        for (j = 0; j < 5; j = j + 1) begin
            if (grant_valid[j]) begin
                p_out_type[j] = p_in_type[grant_input[j]];
                p_out_data[j] = p_in_data[grant_input[j]];
                if (p_in_type[grant_input[j]] == HEAD || p_in_type[grant_input[j]] == HEADTAIL) begin
    ...
    generate
        for (g = 0; g < 5; g = g + 1) begin : pack_out
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (i = 0; i < 5; i = i + 1) begin
                if (ready_i[i] && in_valid[i]) begin
                    if (p_in_type[i] == HEAD) begin
                        route_active[i] <= 1'b1;
                        route_dest[i]   <= want_out[i];
                    end else if (p_in_type[i] == TAIL || p_in_type[i] == HEADTAIL) begin
                        route_active[i] <= 1'b0;
                    end
                end
            end
