...
module tb_link_buffer;
    ...
    localparam DX_W = 4, DY_W = 4, DATA_W = 8, TOTAL_ROUTERS = 4;

    reg clk = 0;
    reg rst_n = 0;
    reg [$clog2(TOTAL_ROUTERS): 0] in_router_code_flit;
    reg in_valid = 0;
    wire in_ready;
    reg  [1:0] in_type = 0;
    ...
    wire [$clog2(TOTAL_ROUTERS): 0] out_router_code_flit;
    wire             out_valid;
    reg              out_ready = 1;
    wire [1:0]       out_type;
    ...
    link_buffer #(.DX_W(DX_W), .DY_W(DY_W), .DATA_W(DATA_W), .TOTAL_ROUTERS(TOTAL_ROUTERS)) dut (
        .clk(clk), .rst_n(rst_n),
    ...
    initial begin
        ...
        out_ready = 1'b1;
        in_router_code_flit = 3'd1; in_valid = 1'b1; in_type = 2'b01; in_dx = 2; in_dy = -1; in_data = 8'hAA;
        #1;
        check(in_ready === 1'b1, "empty buffer accepts immediately");
        @(posedge clk); #1;
        in_router_code_flit = 3'd1; in_valid = 1'b0;
        ...
        out_ready = 1'b1;
        in_router_code_flit = 3'd1; in_valid = 1'b1; in_data = 8'h01;
        ...
        out_ready = 1'b0;
        in_router_code_flit = 3'd1; in_valid = 1'b1; in_data = 8'hBB; in_dx = -3; in_dy = 0;
        #1;
        check(in_ready === 1'b1, "empty buffer still accepts even though downstream isn't ready yet");
        @(posedge clk); #1;
        in_router_code_flit = 3'd1; in_valid = 1'b0; // stop sending -- buffer is now full and downstream is stalled
        ...
        in_router_code_flit = 3'd1; in_valid = 1'b1; in_data = 8'hCC;
        #1;
